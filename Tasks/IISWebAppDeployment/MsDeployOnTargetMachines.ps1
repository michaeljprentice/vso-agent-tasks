...
function Get-MsDeployCmdArgs
{
	...
	$skipAction = $skipAction.Trim('"').Replace('''', '"')
	...
	if(-not (IsInputNullOrEmpty -str $skipAction))
	{
		$msDeployCmdArgs = [string]::Format('{0} -skip:{1}', $msDeployCmdArgs, $skipAction)
	}

	if(-not (IsInputNullOrEmpty -str $overRideParams))
	{
		$msDeployCmdArgs = [string]::Format('{0}{1}', $msDeployCmdArgs, $overRideParams)
	}
...
{    
	...
	$msDeployCmdArgs = Get-MsDeployCmdArgs -webDeployPackage $webDeployPkg -webDeployParamFile $webDeployParamFile -overRideParams $overRideParams -skipAction $skipAction
...
function Decode-String
{
	...
	{
		$decodedString = [System.Text.Encoding]::Unicode.GetString([System.Convert]::FromBase64String($string));
...
function Get-OverRideParameters
{
	...
	$overRideParameters = Decode-String -String $overRideParameters.Trim('"')

	$overRideArgs = [string]::Empty

	[array]$overRideParams = $overRideParameters.Split([System.Environment]::NewLine, [System.StringSplitOptions]::RemoveEmptyEntries)

	foreach($overRideParam in $overRideParams)
	{
		if(-not [string]::IsNullOrEmpty($overRideParam.Trim(' ')))
		{
			if($overRideParam -notmatch "Connection String" -and $overRideParam -notmatch "ConnectionString")
			{
				[array]$splitOverRide  = $overRideParam.Split('=')
				$name = $splitOverRide[0].Trim('"').Trim("'").Trim(' ')
				$value = $splitOverRide[1].Trim('"').Trim("'").Trim(' ')
				
				$cleanOverRide = "`"$name`"=`"$value`""
			}
			else
			{
				$cleanOverRide = $overRideParam.Trim('"').Trim("'").Trim(' ')
				$cleanOverRide = "`"$cleanOverRide`""
			}
			
			$overRideArgs = [string]::Format('{0} -setParam:{1}', $overRideArgs, $cleanOverRide)
			Write-Verbose "Adding additional override. $cleanOverRide" -Verbose
		}
	}
	Write-Verbose "Complete list of overrides. $overRideArgs" -Verbose
	return $overRideArgs
}
...
function Execute-Main
{
	...
	$OverRideParameters = Get-OverRideParameters -overRideParameters $OverRideParams

	Deploy-WebSite -webDeployPkg $WebDeployPackage -webDeployParamFile $WebDeployParamFile -skipAction $SkipAction -overRideParams $OverRideParameters
